package com.junlajobs_backend.helper;

public class CollectionName {

    public static final String COLLECTION_MAIN_COMMENT = "Main_Comment";

    public static final String COLLECTION_SEC0NDARY_COMMENT = "Secondary_Comment";

    public static final String COLLECTION_Portfolio = "Portfolio";

    public static final String COLLECTION_JOBCATEGORY = "Job_category";

    public static final String COLLECTION_JOBTYPE = "Job_type";

    public static final String COLLECTION_LIKE = "Like";

    public static final String COLLECTION_USER = "User";

}
